...
if re.match(SLUG_TEMPLATE, SLUG) is None:
    ...
    SUCCESS = False
    OUTPUT_MSG = f"Your PR title is {SLUG}. This `SLUG` does not match the slug template, which is `2024-\d\d-\d\d-.+`."
...
if len(failed_paths) > 0:
    ...
    OUTPUT_MSG += "ou can only add/change/remove files related to your post, i.e. files that match one of these patterns: `_posts/{SLUG}.md, assets/img/{SLUG}/*,assets/html/{SLUG}/*, assets/bibliography/{SLUG}.bib`. But we found that you changed the following: {'Ɣ'.join(failed_paths)}"
if not SUCCESS:
    OUTPUT += " Also, make sure your PR's title matches your post's `slug`."
    print(OUTPUT_MSG)
